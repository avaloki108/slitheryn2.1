...
from slitheryn.detectors import all_detectors
from slitheryn.detectors.abstract_detector import AbstractDetector, DetectorClassification
from slitheryn.printers import all_printers
from slitheryn.printers.abstract_printer import AbstractPrinter
from slitheryn.slither import Slither
from slitheryn.utils import codex
from slitheryn.utils.output import (
    output_to_json,
    output_to_zip,
    output_to_sarif,
    ZIP_TYPES_ACCEPTED,
...
from slitheryn.utils.output_capture import StandardOutputCapture
from slitheryn.utils.colors import red, set_colorization_enabled
from slitheryn.utils.command_line import (
    FailOnLevel,
    output_detectors,
...
async def _process_with_multi_agent(
    ...
    Returns:
        Tuple of (slither, detector_results, printer_results, analyzed_contracts_count, multi_agent_result)
    """
        ...
        try:
            ...
            for compilation_unit in slither.compilation_units:
                for contract in compilation_unit.contracts:
                    ...
                    try:
                        ...
                        with open(str(contract.source_mapping.filename.absolute), 'r', encoding='utf-8') as f:
                        ...
                        if result and result.get('multi_agent'):
                            ...
                            logger.info(f"   Found {len(result.get('consensus_vulnerabilities', []))} consensus vulnerabilities")
            ...
            if contracts_analyzed:
                ...
                total_consensus_vulns = sum(
                    len(c.get('multi_agent_result', {}).get('consensus_vulnerabilities', []))
...
def main_impl(
    ...
    try:
        ...
        if outputting_json or outputting_zip or outputting_sarif:
